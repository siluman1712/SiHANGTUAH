$(document).ready(function() {
  (function(window, undefined) {
    ...
    var AudioPlayer = (function() {
      ...
      var
        ...
        apActive = false,
        // playlist vars
        plPlaylist = document.getElementById('playlist'),
        pl,
        plUl,
        plLi,
        tplList =
        '<li class="item player_playlist_item pl-list" data-track="{count}">' +
          '<div class="pl-list__track media ">' +
            '<div class="figure avatar30 rounded-4 bg-none pl-list__icon"><i class="fa fa-play"></i></div>' +
            '<div class="song_block pl-list__title  media-body"><h5 class="mt-1">{title}</h5></div>' +
		  	'<div class="pl-list__eq mt-2">' +
              '<div class="eq">' +
      ...
      function init(options) {
       ...
       // repeatBtn = player.querySelector('.repeat_btn');
        ...
        if (settings.autoPlay) {
          ...
          $('#play_circle').removeClass('fa-play text-white border-white').addClass('fa-pause text-danger border-danger');
      ...
      function errorHandler(evt) {
        ...
        $('#play_circle').addClass('fa-play text-white border-white').removeClass('fa-pause  text-danger border-danger');
        $('#npAction').text(function(i, text) {
          if (text === "NOW PLAYING") {
            return "PAUSED...";
          }
      ...
      function renderPL() {
        ...
        pl = create('div', {
          ...
          'id': 'pl',
          'innerHTML': '<ul class="pl-ul list-unstyled userlist">' + (!isEmptyList() ? html.join('') : '<li class="pl-list--empty item">PlayList is empty</li>') + '</ul>'
        });
      ...
      function listHandler(event) {
        ...
        if (event.target.matches('.item')  || event.target.matches('.player_playlist_item') || event.target.matches('.pl-list__track') || event.target.matches('.pl-list__icon') || event.target.matches('.pl-list__eq') || event.target.matches('.eq')) {
          var current = parseInt(event.target.closest('.pl-list').getAttribute('data-track'), 10);
          if (index !== current) {
            index = current;
            play(current);
			  $('#play_circle').addClass('fa-pause text-danger border-danger').removeClass('fa-play text-white border-white');
          } else {
            playToggle();
			   $('#play_circle').addClass('fa-pause text-danger border-danger').removeClass('fa-play text-white border-white');
          }
        } else {
          if (!!event.target.closest('.pl-list__remove')) {
            var parentEl = event.target.closest('.pl-list');
      ...
      function clearAll() {
       ...
       // playSvgPath.setAttribute('d', playSvg.getAttribute('data-play'));
        if (!plUl.querySelector('.pl-list--empty')) {
          plUl.innerHTML = '<li class="pl-list--empty item">PlayList is empty</li>';
        }
        changeDocumentTitle();
      }
  ...
  var iconImage = 'assets/img/user1.png';
...
(function() {
  ...
  (function() {
    $('.back_btn').on('click', function() {		
	 $(this).closest('.card').find('.chat-overlay').toggleClass('active');		
      /*$('.player_playlist').toggleClass('playlist_on');*/		
      $('.fa-angle-left').toggleClass('back_btn_on');
  ...
  (function() {
    $('.play_btn').on('click', function() {
		$('#play_circle').toggleClass('fa-play text-white border-white').toggleClass('fa-pause text-danger border-danger');
      	$('#npAction').text(function(i, text) {
